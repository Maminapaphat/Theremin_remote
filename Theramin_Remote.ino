...
const float dt = 0.005f;    // 5ms loop time (200Hz)
...
//  else Serial.println("esp_now_register_send_cb success");
  ...
  pinMode(1, INPUT);        // INT pin from MPU6050
  pinMode(3, INPUT_PULLUP); // extra octave if pulled down
...
void loop() {
  ...
  if(millis() > lastRead){
    ...
    rawAccX = (buf[0] << 8) | buf[1];     // the 16 bit acc & gyro readings from the MPU6050
    rawAccY = (buf[2] << 8) | buf[3];     // are byte swapped - fix this here
    ...
    // 3. The Complementary Filter - don't jump to the new frequency, slowly sneak up on it
    // Pitch = 98% * (Previous Pitch + Gyro Delta) + 2% * (Accel Pitch)
    pitch = alpha * (pitch + gyroXrate * dt) + (1.0f - alpha) * accPitch;
    if(pitch < -90.0f)  pitch = -90.0f;
    if(pitch >  90.0f)  pitch =  90.0f;   // constrain to range -90 to 90
    float octaveOffset = pitch / (digitalRead(3) ? 45.0f : 30.0f);  // test for extended range option switch
    float freq = 440.0f * exp2f(octaveOffset);  // logarithmic frequency range

    // Roll filtering similar to pitch filtering
    roll  = alpha * (roll + gyroYrate * dt) + (1.0f - alpha) * accRoll; // smooth outthe roll also
    if(roll < 0.0f) roll = 0.0f;
    if(roll >  90.0f) roll =  90.0f;      // constrain to range 0 to 90
    float vol = roll / 90.0f;

    // 4. send pitch, roll, and config bits to Theremin base
    tsi[0] = freq;
    tsi[1] = vol;
    thereminState[8] = digitalRead(5) | digitalRead(6)<<1 | digitalRead(7)<<2;
    thereminState[9] = 0x7d;        // just a constant to test if this is a theremin packet
    esp_now_send(broadcastAddress, thereminState, sizeof(thereminState)); // send to the base
//    return;
